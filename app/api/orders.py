...
from app.table_config import RESERVATION_TABLES

router = APIRouter(prefix="/orders", tags=["orders"])


def resolve_table(table_key: str):
    mapping = RESERVATION_TABLES.get(table_key)
    if not mapping:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Unknown order table '{table_key}'. Valid values: {', '.join(RESERVATION_TABLES)}",
        )
    return mapping


@router.get("/tables")
def list_order_tables() -> dict:
    return {
        key: {
